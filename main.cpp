    ...
    for (unsigned int it_sizes = 0; it_sizes < 5; it_sizes++)
    {
        ...
        for (unsigned int it_perc = 0; it_perc < 7; it_perc++)
        {
            ...
            for (unsigned int it_h = 0; it_h < 100; it_h++)
            {
                ...
                arr.copy();
                test.begin();
                introsort(arr.tab, 0, sizes[it_sizes] - 1);
        ...
        test.begin();
        quicksort(arr.tab, 0, sizes[it_sizes] - 1);
        cout << "\t   Szybkie sortowanie: \t\t" << test.end()*1000.0 << " [ms]" << endl;
        arr.check(sizes[it_sizes]);

        arr.copy();
        test.begin();
        introsort(arr.tab, 0, sizes[it_sizes] - 1);
        cout << "\t   Sortowanie introspektywne: \t" << test.end()*1000.0 << " [ms]" << endl;
        arr.check(sizes[it_sizes]);

        arr.copy();
        test.begin();
        mergesort(arr.tab, 0, sizes[it_sizes] - 1);
        cout << "\t   Sortowanie przez scalanie: \t" << test.end()*1000.0 << " [ms]" << endl << endl;
        arr.check(sizes[it_sizes]);
